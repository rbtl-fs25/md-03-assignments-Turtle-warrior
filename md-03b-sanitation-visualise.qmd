...
```{r}
sanitation_AFG_2000_and_2020_not_safe <- sanitation |> 
  filter(iso3 == "AFG",
         year %in% c(2000, 2020),
...
```{r}
sanitation_AFG_2000_and_2020_not_safe |> 
  count(year)
```
...
```{r}
sanitation_AFG_2000_and_2020_not_safe |> 
  ggplot(data = sanitation_AFG_2000_and_2020_not_safe,
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    The order is according to the legend and not to the size

2.  What would you want to change?

    I would change it so that it goes from the biggest part to the smallest part from bottom to top

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    That's the uninteresting part (no work to do)
...
2.  What do you observe when you look at the code and plot?
    ...
    There's many measurements where the basic san has a high percentage and then more distributed for safely managed san, could mean that there is a big gap between basic and save and not much inbetween.
